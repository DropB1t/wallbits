...
convert_png_to_jpg() {
    ...
    # Use mapfile to read the output of find into an array with png and jpeg extensions
    mapfile -t png_files < <(find "$walls_dir" -type f \( -iname "*.png" -o -iname "*.jpeg" \))
    ...
    for png_file in "${png_files[@]}"; do
        # Generate JPG filename, replacing extension after . with .jpg
        ...
        if convert "$png_file" -quality 95 "$jpg_file"; then
            ...
            rm "$png_file"  # Remove the original PNG file
            count=$((count + 1))
        else
